// File: src/pages/UnifiedMonthlyKPIEditor.tsx
...
const UnifiedMonthlyKPIEditor: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function loadItems() {
      try {
        ...
        const filtered = response.data.value.filter((item: any) => item.fields.Project === selectedProject);
        setItems(filtered);

        const yearSet = new Set(filtered.map((item: any) => item.fields.year));
        setYears(Array.from(yearSet) as string[]);
  ...
  return (
    <div className="relative w-full min-h-screen bg-cover bg-center text-white" style={{ backgroundImage: `url(${harnessBg})` }}>
      <div className="relative z-20 max-w-6xl mx-auto p-6">
        ...
        <h1 className="text-xl mt-4 mb-6 font-bold text-white/90">Unified KPI Editor</h1>
            ...
            <table className="w-full border border-white/20 text-sm text-white bg-white/10">
              <thead>
                <tr>
                  ...
                  <th className="p-2 border">Production Minutes</th>
                  <th className="p-2 border">Downtime</th>
                ...
                {items
                  ...
                  .map((itm) => (
                    <tr key={itm.id} className="border-t border-white/10">
                      ...
                      <td className="p-2 border">{itm.fields.productionminutes}</td>
                      <td className="p-2 border">{itm.fields.downtime}</td>
                      <td className="p-2 border">{itm.fields.rateofdowntime.toFixed(3)}</td>
                      <td className="p-2 border">{itm.fields.Targetdowntime}</td>
                      <td className="p-2 border">{itm.fields.seuildinterventiondowntime}</td>
...
export default UnifiedMonthlyKPIEditor;
